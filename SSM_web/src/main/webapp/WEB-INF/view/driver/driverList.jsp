    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('#dataList').datagrid({
                ...
                fit: true,//自动大小
                method: "post",
                url: "getDriverList?t" + new Date().getTime(),
                idField: 'id',
            ...
            $("#delete").click(function () {
                ...
                if (selectLength === 0) {
                    ...
                    $.messager.confirm("消息提醒", "删除后将无法恢复该司机信息! 确定继续?", function (r) {
                        if (r) {
                            $.ajax({
                                type: "post",
                                url: "deleteDriver?t" + new Date().getTime(),
                                data: {ids: ids},
                                dataType: 'json',
                ...
                buttons: [
                    {
                        ...
                        handler: function () {
                            ...
                            if (!validate) {
                                ...
                                $.ajax({
                                    type: "post",
                                    url: "addDriver?t" + new Date().getTime(),
                                    data: data,
                                    dataType: 'json',
                                    success: function (data) {
                                        if (data.success) {
            ...
            $("#editDialog").dialog({
                ...
                buttons: [
                    {
                        ...
                        handler: function () {
                            ...
                            if (!validate) {
                                ...
                                $.ajax({
                                    type: "post",
                                    url: "editDriver?t=" + new Date().getTime(),
                                    data: data,
                                    dataType: 'json',
                                    success: function (data) {
                                        if (data.success) {
                ...
                onBeforeOpen: function () {
                    ...
                    $("#edit_id").val(selectRow.id);//初始化id值,需根据id更新司机信息
            ...
            //司机与线路名搜索按钮的监听事件(将其值返回给Controller)
            $("#search-btn").click(function () {
                $('#dataList').datagrid('load', {
                    drivername: $('#search-drivername').val(),
                    routename: $('#search-routename').combobox('getValue')
                });
            });
        ...
        function uploaded() {
            ...
            if (data.success) {
                ...
                //将头像路径存储到司机信息表单中(利用从用户信息中读取头像路径来显示头像)
...
<!-- 工具栏 -->
<div id="toolbar">
    <%-- 通过JSTL设置用户操作权限: 将增加、修改和删除按钮设置为仅管理员 --%>
    ...
    <div style="margin-left: 10px;">
        ...
        <select id="search-routename" style="width: 155px;" class="easyui-combobox" name="routename">
            <!-- 通过JSTL遍历显示片区信息,routeList:为Contrller传递的来的,存储着Route的List对象 -->
            <option value="">未选择线路</option>
        ...
        <!-- 司机姓名搜索框 -->
        <a href="javascript:" class="easyui-linkbutton"
           data-options="iconCls:'icon-user-student',plain:true">司机名称</a>
        <input id="search-drivername" class="easyui-textbox" name="drivername"/>
        <!-- 搜索按钮 -->
...
<div id="addDialog" style="padding: 15px 0 0 55px;">
    ...
    <div style="float: right; margin: 15px 40px 0 0; width: 250px; border: 1px solid #EEF4FF" id="add-photo">
        <img id="add-portrait" alt="照片" style="max-width: 250px; max-height: 300px;" title="照片"
             src="${pageContext.request.contextPath}/image/portrait/default_driver_portrait.png"/>
        <!-- 设置上传图片按钮 -->
    ...
    <form id="addForm" method="post" action="#">
        <table id="addTable" style="border-collapse:separate; border-spacing:0 3px;" cellpadding="6">
            ...
            <tr>
                <td>线路</td>
                <td colspan="1">
                    <select id="add_route_name" style="width: 200px; height: 30px;" class="easyui-combobox"
                            name="route_name" data-options="required:true, missingMessage:'请选择所属线路~'">
                        <c:forEach items="${routeList}" var="route">
                            <option value="${route.name}">${route.name}</option>
                        </c:forEach>
                    </select>
            ...
            <tr>
                ...
                <td>
                    <select id="add_gender" class="easyui-combobox"
                            ...
                            required:true, missingMessage:'请选择性别'" name="gender">
            ...
            <tr>
                ...
                <td colspan="1">
                    <input id="add_sno" style="width: 200px; height: 30px;" class="easyui-textbox"
                           type="text" name="sno" data-options="required:true, missingMessage:'请填写车牌号'"/>
                </td>
            </tr>
            <tr>
                <td>密码</td>
                <td colspan="1">
                    <input id="add_password" style="width: 200px; height: 30px;" class="easyui-textbox"
                           type="password" name="password" data-options="required:true, missingMessage:'请填写密码'"/>
                </td>
            </tr>
            <tr>
                <td>邮箱</td>
            ...
            <tr>
                ...
                <td colspan="4"><input id="add_introducation" style="width: 200px; height: 60px;" class="easyui-textbox"
                                       ...
                                       data-options="multiline:true,required:true, missingMessage:'请填写备注信息！'"/>
                </td>
            </tr>
        </table>
    </form>
    ...
    <form id="editForm" method="post" action="#">
        ...
        <table id="editTable" style="border-collapse:separate; border-spacing:0 3px;" cellpadding="6">
            ...
            <tr>
                <td>线路</td>
                <td colspan="1">
                    <select id="edit_route_name" style="width: 200px; height: 30px;" class="easyui-combobox"
                            name="route_name">
                        <c:forEach items="${routeList}" var="route">
                            <option value="${route.name}">${route.name}</option>
                        </c:forEach>
